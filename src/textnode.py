...
def split_nodes_delimiter(old_nodes, delimiter, text_type):
    ...
    for old_node in old_nodes:
        if old_node.text_type != TextType.TEXT:
            ...
            if len(split) % 2 == 0:
                raise Exception("invalid markdown syntax, no closing deliminator found")
...
def text_to_textnodes(text):
    starting_nodes = [TextNode(text, TextType.TEXT)]
    bold_nodes = split_nodes_delimiter(starting_nodes, "**", TextType.BOLD)
    italic_nodes = split_nodes_delimiter(bold_nodes, "_", TextType.ITALIC)
    italic_nodes_2 = split_nodes_delimiter(italic_nodes, "*", TextType.ITALIC)
